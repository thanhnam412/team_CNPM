...
export class ProposalsService {
  ...
  async createProposal(
    ...
    if (!quickTaskId && !milestoneId) {
      throw new BadRequestException('Proposal must target a QuickTask or a Milestone.');
    }
    ...
    if (quickTaskId) {
      ...
      if (quickTask.status !== 'OPEN') {
        throw new BadRequestException('This QuickTask is no longer open for proposals.');
      }
    ...
    if (milestoneId) {
      ...
      if (milestone.status !== 'PENDING') {
        throw new BadRequestException('This Milestone is no longer accepting proposals.');
      }
      ...
      if (adminMember?.userId === expertId) {
        throw new BadRequestException('You cannot propose to your own project milestone.');
      }
    }
  ...
  async resolveClientAndPrice(
    ...
    if (proposal.quickTaskId) {
      ...
      if (!member) throw new BadRequestException('No CLIENT_ADMIN found for this project.');
      clientId = member.userId;
    } else {
      throw new BadRequestException('Proposal has no target (quickTaskId or milestoneId).');
    }
  ...
  async updateStatus(
    ...
    if (status !== 'REJECTED' && status !== 'WITHDRAWN') {
      throw new BadRequestException('Status can only be updated to REJECTED or WITHDRAWN via this endpoint.');
    }
    ...
    if (status === 'WITHDRAWN') {
      if (proposal.expertId !== actorId) {
        throw new ForbiddenException('Only the expert can withdraw their proposal.');
      }
    } else if (status === 'REJECTED') {
      const { clientId } = await this.resolveClientAndPrice(proposal, this.db);
      if (clientId !== actorId) {
        throw new ForbiddenException('Only the client can reject this proposal.');
      }
    }
