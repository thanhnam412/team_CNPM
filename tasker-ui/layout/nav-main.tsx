...
function NavItem({ item }: { item: NavItem }) {
  ...
  // Root paths should only match exactly to prevent them from always being active
  const isRootPath = item.url === "/client" || item.url === "/expert";
  const isActive =
    pathname === item.url ||
  ...
  return (
    ...
    >
      ...
      <CollapsibleContent>
        <SidebarMenuSub className="ml-4 mt-0.5 gap-2 border-l-2 border-border pl-2">
